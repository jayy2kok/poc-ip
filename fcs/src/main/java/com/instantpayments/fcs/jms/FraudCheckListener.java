...
/**
 * Listens on fraud.request.queue for XML fraud check requests,
 * processes them, and sends XML responses to fraud.response.queue.
 */
@Component
public class FraudCheckListener {
    ...
    public FraudCheckListener(FraudCheckEngine fraudCheckEngine, JmsTemplate jmsTemplate) throws Exception {
    ...
    public void onFraudCheckRequest(String xmlMessage) {
        try {
            ...
            FraudCheckRequest request = (FraudCheckRequest) unmarshaller.unmarshal(new StringReader(xmlMessage));
            ...
            log.info("Sent fraud check response for transaction {} to {}: {}",
                    request.getTransactionId(), FRAUD_RESPONSE_QUEUE, response.getOutcome());
