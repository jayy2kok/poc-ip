...
/**
 * Listens on payment.notification.queue for fraud check results from BS. Updates payment status and
  ...
  public void onPaymentNotification(String jsonMessage) {
    try {
      ...
      PaymentNotification notification =
          objectMapper.readValue(jsonMessage, PaymentNotification.class);
      ...
      Payment payment =
          paymentService.updateStatus(
              notification.getTransactionId(), newStatus, notification.getMessage());

      // Send WebSocket notification (status only, no fraud details)
      notificationService.sendStatusUpdate(
          payment.getTransactionId(), payment.getStatus().name(), "PENDING");

      log.info(
          "Processed notification for transaction {}: {}",
