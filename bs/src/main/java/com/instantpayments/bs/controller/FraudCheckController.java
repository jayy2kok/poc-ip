...
/**
 * Solution 2: REST-based inbound from PPS. Receives JSON payment payload via REST, converts to XML,
 * sends to FCS via JMS.
 */
@RestController
@Profile("sol2")
public class FraudCheckController implements FraudCheckApi {
  ...
  public ResponseEntity<FraudCheckAcceptedResponse> submitFraudCheck(
    ...
    log.info(
        "Forwarded fraud check to FCS for transaction {}", fraudCheckRequest.getTransactionId());
