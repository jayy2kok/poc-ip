...
/**
 * Listens on fraud.response.queue for XML responses from FCS,
 * converts XML→JSON, and sends notification to payment.notification.queue.
 */
@Component
public class FraudResponseListener {
    ...
    public FraudResponseListener(XmlJsonConverter xmlJsonConverter, JmsTemplate jmsTemplate, ObjectMapper objectMapper) {
    ...
    public void onFraudResponse(String xmlMessage) {
        try {
            ...
            log.info("Sent payment notification for transaction {} to {}: {}",
                    notification.getTransactionId(), QueueNames.PAYMENT_NOTIFICATION_QUEUE,
