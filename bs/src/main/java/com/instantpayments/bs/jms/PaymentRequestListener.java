...
/**
 * Solution 1: JMS-based inbound from PPS.
 * Listens on payment.request.queue, converts JSON→XML, sends to fraud.request.queue.
 */
@Component
@Profile("sol1")
public class PaymentRequestListener {
    ...
    public PaymentRequestListener(JsonXmlConverter jsonXmlConverter, JmsTemplate jmsTemplate, ObjectMapper objectMapper) {
    ...
    public void onPaymentRequest(String jsonMessage) {
        try {
            ...
            log.info("Forwarded fraud check request for transaction {} to {}",
                    payload.getTransactionId(), QueueNames.FRAUD_REQUEST_QUEUE);
